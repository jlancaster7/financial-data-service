...
def test_basic_snowflake_connection():
    ...
    try:
        ...
        if "EQUITY_DATA_LOADER" in available_roles:
            ...
            logger.warning(
                "⚠ EQUITY_DATA_LOADER role does not exist - need to run setup"
            )
        ...
        if config.snowflake.database in db_names:
            ...
            logger.warning(
                f"⚠ Database {config.snowflake.database} does not exist - need to run setup"
            )
...
def main():
    ...
    if snowflake_ok:
        logger.info("\nNext steps:")
        logger.info(
            "1. Run 'python scripts/setup_snowflake.py' to create database and roles"
        )
        logger.info(
            "2. Then run 'python scripts/test_connections.py' to verify full setup"
        )
