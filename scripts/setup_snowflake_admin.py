...
def setup_snowflake():
    ...
    try:
        ...
        cursor = conn.cursor()
        try:
            cursor.execute(f"GRANT ROLE EQUITY_DATA_LOADER TO USER {config.snowflake.user}")
            cursor.execute(f"GRANT ROLE EQUITY_DATA_READER TO USER {config.snowflake.user}")
        ...
        try:
            ...
            cursor.execute(f"SELECT COUNT(*) FROM {config.snowflake.database}.ANALYTICS.DIM_DATE")
